...
double (NOT FLOAT) 1.234f float 'x'    char
                                ...
                                The sizeof operator reports how many bytes a
type or variable is. For example cout << sizeof(int); should print 4
...
cannot mix. If we combine two different types, the one that is less capable is
"promoted"

For example:
   double x =  3 + 4.2   //3 is converted to 3.0
   double y = 3 / 2;     // first compute integer 3 / 2 which is 1, THEN convert
to 1.0 uint32_t z = -1;      // convert the bits of -1
(11111111111111111111111111) to unsigned 32-bit
...
3b. floating point errors: roundoff errors, numbers not exactly representable
   ...
   The terms sign bit, exponent, and mantissa
         Some numbers we think as "nice" like 1/10 are not finite in binary
because 1/5 is a repeating fraction.

         Equality is useless with floating point numbers. Testing a == b or a !=
b is almost always not going to work. Instead we use approximations and
...
Example problems:
  Write a recursive implementation of factorial that can compute up to
factorial(100) without overflow.
...
uint64_t prod(uint64_t a, uint64_t b) {
  return a * b;
}  // overflow, everything has to be 64 bit for passed parameters to work

uint32_t sum(int n) {  // uninitialized variable
  int s = 0;
...
bool yes_or_no(
    float sumin,
    uint32_t n) {  // another floating point comparison, floating points will
                   // never equal each other because of roundoff. therefore you
                   // have to check for precision instead of equality
  float sum = 0;
  float x = 0;
  for (int i = 0; i < n; i++) sum += .1;
  return abs(sum - sumin) < .00001;
}

double average(
    int x[], int n) {  // bounds check, for loop goes to x[4] which doesnt exist
  double sum = 0;
...
double f(double x) {
  return x * x - 3;
}  // different types being used, for some reason 3 has to have .0 for this to
   // compute as floating point

double count(double a, double b,
             uint32_t num_steps) {  // floating point comparison, will never
                                    // equal each other because of roundoff
  const double dx = (b - a) / num_steps;
  for (int x = 1; x < num_steps; x++) {
    cout << a << ' ';
    a += dx;
  }
  return a;
...
void bugs() {
  cout << f(3.1) << endl;
  uint64_t ans = prod(1000000, 2000000);
  uint64_t big =
      50000ULL *
      50000;  // actual integers being multiplied are by default signed
              // integers, have to put ULL on one of them for this to work
  cout << ans << '\n';
  cout << "sum(10)=" << sum(10) << '\n';
...
uint64_t cubeit(uint64_t a, uint64_t b) {
  uint64_t sum = 0;
  for (int i = a; i <= b; i++) {
    sum = sum + (i * i * i);
  }
...
double diffsquares(const double d[], int size) {
  double sumSquares = 0;
  double squareSum = 0;
  for (int i = 0; i < size; i++) {
    sumSquares = sumSquares + d[i] * d[i];
  }
  for (int i = 0; i < size; i++) {
    squareSum = squareSum + d[i];
  }
...
uint64_t factorial(uint64_t start) {
  if (start == 1) {
    return 1;
  } else {
